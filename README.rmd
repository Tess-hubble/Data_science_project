...
training_rf
default_rmse <- sqrt(training_rf$prediction.error) # RMSE: 2247.12
...
rmse_rf <- sqrt(mean((predictions - testing_data$auction_tot)^2)) # RMSE:2474
...
importance_df <- data.frame(
  Feature = colnames(training_data)[1:ncol(training_data) - 1],
  Importance = as.numeric(training_rf$variable.importance)
)

add_men <- c("men_total", 431116.1081)
importance_df <- rbind(importance_df, add_men) %>% 
    arrange(desc(Importance))

importance_df <- importance_df[order(-importance_df$Importance), ]
ggplot(importance_df, x= Feature y = Importance)) +
