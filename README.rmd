...
train_size <- floor(0.6 * nrow(ml_df_log))
training_data <- ml_df_log[indices[1:train_size], ]
testing_data <- ml_df_log[indices[(train_size + 1):nrow(ml_df_log)], ]
...
```{r}
ml_df_log <- data.frame(lapply(ml_df, function(x) if (is.numeric(x) && all(x > 0)) log(x) else x))
dim(ml_df)
...
training_rf
default_rmse <- sqrt(training_rf$prediction.error) # RMSE: 2389.86
